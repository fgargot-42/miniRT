...
/*   Updated: 2026/07/03 19:52:19 by mabarrer         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_editor(t_data *data)
{
	...
	if (!data->editor)
		exit(1);
    mlx_on_event(data->mlx, data->editor, MLX_WINDOW_EVENT, editor_close_hook, data);
...
//		- (SLD_BASE_Y - 44)}}, (mlx_color){.rgba = COL_BG});
	 fill_rect(data,
			 (t_vec2){{0, 0}},
			 (t_vec2){{EDITOR_W, EDITOR_H}},
			 (mlx_color){.rgba = COL_BG});
	 header(data, mouse_x, mouse_y);
	 y = SLD_BASE_Y - 130;
