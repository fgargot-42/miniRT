...
/*   Updated: 2026/04/28 23:03:40 by fgargot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	hit_cylinder_cap(t_cylinder *cyl, t_hit_ctx *ctx)
{
	...
	v_len = cyl->height / 2.0;
	if (ctx->oc.z < 0)
		v_len = -cyl->height / 2.0;
	if (fabs(ctx->oc.z) < cyl->height / 2.0)
	{
		if (vec3_length(vec3_multiply(ctx->oc, z_scale)) > cyl->radius + 1e-3)
			return (0);
	...
	if (vec3_length(vec3_multiply(v_hit_cap, z_scale)) > cyl->radius + 1e-3)
		return (0);
	if (fabs(v_hit_cap.z) > cyl->height / 2.0 + 1e-3 || v_len < T_MIN
