...
/*   Updated: 2026/04/17 23:07:58 by fgargot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_hit_ctx ctx)
{
	...
	if (fabs(fabs(ctx.render_hit.z) - cyl->height) < 1e-6)
		rec->normal = (t_vec3){0, 0, z_cap};
	else
...
static int	hit_cylinder_cap(t_cylinder *cyl, t_hit_ctx *ctx)
{
	...
	if (fabs(ctx->rd.z) < 1e-6)
		return (0);
	if (fabs(ctx->oc.z) > cyl->height / 2.0
		&& ((ctx->oc.z > 0) == (ctx->rd.z > 0)))
		return (0);
	is_inside = fabs(ctx->oc.z) < cyl->height / 2.0
		&& vec_length(vec_multiply(ctx->oc, z_scale)) < cyl->radius;
	v_len = cyl->height / 2.0;
	if ((is_inside && ctx->rd.z < 0) || (!is_inside && ctx->oc.z < 0))
		v_len *= -1;
