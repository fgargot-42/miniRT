...
/*   Updated: 2026/05/14 00:31:13 by fgargot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_cylinder_aabb(t_object *obj, t_vec3 *aabb_min, t_vec3 *aabb_max)
{
	...
	if (!aabb_min || !aabb_max)
		return ;
	 extent.x = obj->props.height * fabs(obj->direction.x) + obj->radius *
	 	sqrt(1 - pow(obj->direction.x, 2));
	 extent.y = obj->props.height * fabs(obj->direction.y) + obj->radius *
	 	sqrt(1 - pow(obj->direction.y, 2));
	 extent.z = obj->props.height * fabs(obj->direction.z) + obj->radius *
...
void	get_cone_aabb(t_object *obj, t_vec3 *aabb_min, t_vec3 *aabb_max)
{
	...
	while (i < 2)
	{
		extent[i].x = height[i] * fabs(obj->direction.x) + obj->radius *
			sqrt(1 - pow(obj->direction.x, 2));
		extent[i].y = height[i] * fabs(obj->direction.y) + obj->radius *
			sqrt(1 - pow(obj->direction.y, 2));
		extent[i].z = height[i] * fabs(obj->direction.z) + obj->radius *
			sqrt(1 - pow(obj->direction.z, 2));
		i++;
	}
